library("Biostrings")
library(seqRFLP)     #to tranform a dataframe in FASTA file

fasta <- readDNAStringSet("PacBio_sample1.fasta")  #FASTA file of PacBio long reads
#head(fasta)
fasta.df <- data.frame(as.character(fasta))        #transform the FASTA file in a data frame to handle data better


fusions <- readDNAStringSet("short_reads_fusion.txt")   #file containing the fusion transcripts identified by short reads
seq_name = names(fusions)
sequence = paste(fusions)
df.fusions <- data.frame(seq_name, sequence)            #transform the fusion transcripts data (short_reads_fusion.txt) in a dataframe


#For each short_read_fusion, find matches in long reads data (FASTA file)
for (i in 1:length(sequence)){
  #print(i)                                            
  seq <- (sequence[i])
  print(sequence[i])
  fusion_name <- seq_name[i]
  match <- vmatchPattern(seq,fasta,max.mismatch=2)
  match.df <- data.frame(as.character(unlist(match)))
  seq.matched <-subset(fasta.df,rownames(fasta.df) %in% rownames(match.df))
  if (nrow(seq.matched) != 0) {
    fasta.match<- dataframe2fas(seq.matched, file = paste(fusion_name,".fasta", sep =''))
  }
}


